...
class logMelFb(nn.Module):
    def __init__(self, options, inp_dim):
        ...
        self._mspec = torchaudio.transforms.MelSpectrogram(
            ...
            win_length=self._stft_window_size,
...
class SincConv(nn.Module):
    ...
    def __init__(
        ...
        if in_channels != 1:
            # msg = (f'SincNet only support one input channel '
            #       f'(here, in_channels = {in_channels:d}).')
            msg = "SincNet only support one input channel (here, in_channels = {%i})" % (in_channels)
        ...
        if bias:
            raise ValueError("SincNet does not support bias.")
        if groups > 1:
            raise ValueError("SincNet does not support groups.")
...
class PASE(nn.Module):
    def __init__(self, options, inp_dim):
        ...
        from \
            pase.models.frontend import wf_builder
